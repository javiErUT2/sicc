...
<div id="update{{$student->id}}" class="modal_edit">
    <div  class="modal__container grid gap-4  bg-black  sombrita  ">
        <form  enctype="multipart/form-data" action="{{url('estudiantes',$student->id)}}" method="POST">
            <h1 class="font-bold  text-[30px] sombrita-txt text-[#000000]">Editar Alumno</h1>
            @csrf
            @method('PUT')
            ...
            <div class="flex flex-row gap-2">
                <button class="bg-[#00ab84] p-2 rounded hover:opacity-50 transition-all hover:scale-110 text-white mt-2" type="submit">
                    Actualizar
                </button>
...
<div  id="delete{{$student->id}}" class="modal_delete">
    <div class="modal__container   sombrita  ">
        <form class="grid  gap-4 p-10 text-center" enctype="multipart/form-data" action="{{ url('estudiantes',$student->id) }}" method="POST">
            <h1 class="font-bold text-[30px] sombrita-txt text-[#070708]">Eliminar Alumno</h1>
            @csrf
            @method('DELETE')
            <div class="grid place-items-center ">
                <div>
                    <h1 class="font-bold text-[30px] sombrita-txt text-[#00ab84]">¿Estás seguro de eliminar al alumno: {{ $student->student_name }} {{ $student->last_name }}?</h1>
                </div>
                <div>
                <button class="bg-[#ff3c3c] p-2 rounded hover:opacity-50 transition-all hover:scale-110 text-white mt-2" type="submit">
                    Sí
                </button>
